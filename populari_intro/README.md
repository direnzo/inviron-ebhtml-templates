# populari_video_primeiro

Variante do template **populari** com a **ordem invertida**: o video de
abertura toca PRIMEIRO, em tela cheia, e so quando ele termina a noticia
(foto + texto) e revelada. No template `populari` original a ordem e a
inversa (noticia -> video de encerramento).

Mesmos assets, mesmo dataset e mesmo CSS do `populari`; a diferenca esta
no `js/master.js` (fluxo) e em pequenos ajustes de `css/master.css`
(classe `.news-in` controla a entrada da noticia).
...
## Ciclo de playlist (ordem invertida)

1. Imagem carrega (ou watchdog `IMAGE_TIMEOUT_MS`) -> `loader.loaded()` e o
   corpo fica visivel (`is-ready`), mas a noticia ainda esta oculta.
2. Entra **primeiro** o video de abertura em tela cheia (`#vinheta`),
   escolhido pelo formato:
...
3. Quando o video termina (`ended`, `timeupdate` no fim, ou watchdog
   `VIDEO_TIMEOUT_MS`), o body recebe `.news-in`: o video some em fade e a
   noticia (foto + zoom + degrade + texto) entra.
4. Passados `DURATION_MS` com a noticia no ar, `loader.finished()` libera a
   playlist.

Sem dados / sem item: `loaded()` + `finished()` sem video (igual ao
`populari`).
...
3. `onerror` / watchdog `VIDEO_TIMEOUT_MS` -> `revelarNoticia()` garantido
   (revela a noticia e agenda `loader.finished()`), a playlist nunca trava.

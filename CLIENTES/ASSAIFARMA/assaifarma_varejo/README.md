...
- **Causa raiz real de "item toca e some quase na hora" (produção)**: a 3ª consulta (`readConfig`/`loaderConfig`, só para `IMAGE_LOGO`, que este template nem usa — `#logo_container` fica sempre invisível) retornava 0 registros no canal real. O `interface` do `EBBrowser` é **global por página** (não por instância de loader): quando esse loader secundário bate em 0 registros com `nodataiserror=false`, o `ebhtml.js` chama `this.browser.finished()` internamente, disparando o `ebfinished()` **real** pro player nativo — encerrando o item de verdade e sobrepondo o `setTimeout(CONFIG.timing.duration)` do loader principal. Removida essa 3ª consulta por completo; o logo agora é lido do mesmo registro do produto já carregado (`aplicarLogo(data2.get(idx))`). Documentado em `.github/copilot-instructions.md` (seção 1.2) e `.github/skills/ebhtml-api/SKILL.md` para uso em templates futuros.

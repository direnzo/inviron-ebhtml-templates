...
function ajustarFontePreco(textoPreco) {
    ...
    document.getElementById('preco-simbolo').style.fontSize = (vw * 0.50) + 'vw';
    // "POR:" acompanha proporção de "R$"
    var porLabel = document.getElementById('preco-por-label');
    if (porLabel) porLabel.style.fontSize = (vw * 0.32) + 'vw';
}
...
function renderizar(produto, loader) {
    ...
    var hasFidelidade = produto.fidelidade != null;
    var fidelidadeBloco = document.getElementById('fidelidade');

    // Background image: fidelidade usa bg_oferta, demais usam bg_aproveite
    if (hasFidelidade) {
    ...
    if (hasFidelidade) {
        var fid = produto.fidelidade;
        document.getElementById('fidelidade-leve').innerText = 'leve ' + fid.quantity;
        document.getElementById('fidelidade-total').innerText = formatarPreco(fid.totalPrice);
        document.getElementById('fidelidade-unitario-promo').innerText = 'preço unitário na promoção R$ ' + formatarPreco(fid.unitPrice);
        document.getElementById('fidelidade-unitario-regular').innerText = 'preço unitário sem promoção R$ ' + formatarPreco(produto.regularPrice);
        fidelidadeBloco.style.display = 'flex';
    ...
    } else if(produto.regularPrice != null && produto.regularPrice != undefined) {
        // Preço regular
